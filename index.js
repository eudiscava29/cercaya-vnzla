...
// Adaptador universal para Cloudflare D1 mediante fetch a su API REST o bindings
const d1Client = {
  async execute(sql, params = []) {
    // Si estás corriendo en Cloudflare Workers con env.DB
    ...
    // Si estás corriendo en Render conectado a Cloudflare D1 vía API REST
    ...
    // D1 devuelve un array de resultados para las consultas
...
const pool = {
  async query(text, params = []) {
    ...
    // Adaptador para transformar la sintaxis PostgreSQL ANY() a IN () compatible con SQLite/D1
    if (sql.includes('ANY($1::int[])') && Array.isArray(params[0])) {
      ...
      // Reemplazar $1, $2 por ? estándar de SQLite
      let index = 1;
    ...
    const res = await d1Client.execute(sql, args);
    return res;
  }
};
...
async function conectarDB() {
  try {
    ...
    const plansCheck = await pool.query('SELECT COUNT(*) as count FROM plans');
...
app.get('/citizen-dashboard', async (req, res) => {
  ...
  const userRes = await pool.query('SELECT * FROM users WHERE id = ? AND role = ?', [userId, 'user']);
...
app.post('/api/login', async (req, res) => {
  ...
  const userRes = await pool.query('SELECT * FROM users WHERE email = ? AND password = ?', [email, password]);
  const user = (userRes.rows || [])[0];
  ...
  const bizRes = await pool.query('SELECT * FROM businesses WHERE user_id = ?', [user.id]);
...
app.get('/forgot', async (req, res) => {
  ...
  const userRes = await pool.query('SELECT * FROM users WHERE email = ?', [emailQuery]);
  const user = (userRes.rows || [])[0];
...
app.get('/dashboard', async (req, res) => {
  const userId = req.query.user_id;
  const bizRes = await pool.query('SELECT * FROM businesses WHERE user_id = ?', [userId]);
  const business = (bizRes.rows || [])[0];
  ...
  if (business.status === 'blocked' || (business.expires_at && now > new Date(business.expires_at).getTime())) {
    await pool.query('UPDATE businesses SET status = ? WHERE id = ?', ['blocked', business.id]);
  ...
  const prodRes = await pool.query('SELECT * FROM products WHERE business_id = ?', [business.id]);
...
app.get('/admin', async (req, res) => {
  ...
  if (activeTab === 'citizen') {
    const citRes = await pool.query('SELECT * FROM users WHERE role = ?', ['user']);
    ...
    if (activeTab === 'trial') {
      ...
      bizRes = await pool.query("SELECT * FROM businesses WHERE plan_id = ?", [parseInt(activeTab)]);
    }
...
app.post('/api/admin/edit-citizen', async (req, res) => {
  ...
  await pool.query('UPDATE users SET full_name = ?, phone = ? WHERE id = ?', [full_name, phone, user_id]);
...
app.post('/api/admin/edit-business', async (req, res) => {
  ...
  await pool.query('UPDATE businesses SET name = ?, phone = ? WHERE id = ?', [name, phone, business_id]);
...
app.post('/api/merchant/rate', async (req, res) => {
  ...
  const bizRes = await pool.query('SELECT * FROM businesses WHERE id = ?', [business_id]);
  const biz = (bizRes.rows || [])[0];
  if (biz) {
    await pool.query('UPDATE businesses SET bcv_rate = ? WHERE id = ?', [parseFloat(bcv_rate), biz.id]);
...
app.post('/api/merchant/product', async (req, res) => {
  ...
  const countRes = await pool.query('SELECT COUNT(*) as count FROM products WHERE business_id = ?', [biz.id]);
  ...
  await pool.query('INSERT INTO products (business_id, title, category, price_usd) VALUES (?, ?, ?, ?)', [biz.id, title, category, parseFloat(price_usd)]);
...
app.post('/api/merchant/product/edit', async (req, res) => {
  ...
  await pool.query('UPDATE products SET title = ?, price_usd = ? WHERE id = ?', [title, parseFloat(price_usd), product_id]);
  res.redirect(`/dashboard?user_id=${user_id}`);
});
...
app.post('/api/admin/unlock', async (req, res) => {
  ...
  await pool.query('UPDATE businesses SET status = ?, expires_at = ? WHERE id = ?', ['active', next.toISOString(), business_id]);
  res.redirect('/admin?tab=trial');
});
...
app.post('/api/admin/unlock-citizen', async (req, res) => {
  ...
  const next = new Date();
  next.setDate(next.getDate() + 30);
  await pool.query('UPDATE users SET status = ?, expires_at = ? WHERE id = ?', ['active', next.toISOString(), user_id]);
  res.redirect('/admin?tab=citizen');
});
...
app.get('/api/search', async (req, res) => {
  ...
  const prodRes = await pool.query(`SELECT * FROM products WHERE business_id IN (${placeholders})`, bizIds);
  const products = prodRes.rows || [];
  let results = [];
...
app.listen(PORT, '0.0.0.0', () => { console.log(`🚀 CercaYa con Cloudflare D1 activo en puerto ${PORT}`); });
