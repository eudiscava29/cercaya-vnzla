            ...
            html += \`
              <div class="bg-white rounded-2xl p-4 shadow-sm border \${index === 0 ? 'border-emerald-400 ring-2 ring-emerald-100' : 'border-slate-100'} relative">
                ...
                <div class="mt-3 text-right">
                  <a href="\${mapsUrl}" target="_blank" class="inline-block bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold px-3 py-2 rounded-xl transition shadow-sm">🗺️ Ir al negocio en Google Maps</a>
                </div>
              </div>
            \`;
          });
...
app.post('/api/register', async (req, res) => {
  ...
  if (parseInt(plan_id) !== 1 && business_name) {
    await pool.query(
      ...
      [user.account_id, user.id, business_name, full_name, category || 'Otros', reference || 'Céntrico', lat_lng || '10.2241,-67.5871', phone, parseInt(plan_id), 36.50, 'trialing', new Date(Date.now() + 14*86400000).toISOString()],
      req
    );
  }
  res.redirect('/login');
