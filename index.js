...
app.post('/api/admin/login', (req, res) => {
  ...
  const adminUser = process.env.ADMIN_USER || 'Eudis';
  const adminPass = process.env.ADMIN_PASS || 'mercy123';
...
// ==================== BÚSQUEDA OPTIMIZADA (SQL JOIN DIRECTO) ====================
app.get('/api/search', async (req, res) => {
  const query = req.query.query || '';
  const catFilter = req.query.cat || '';
  
  // Optimizada mediante JOIN directo en SQL para evitar procesar toda la data en memoria del servidor
  let sql = `
    SELECT p.*, b.name as business_name, b.lat_lng, b.phone, b.bcv_rate 
    FROM products p
  ...
  if (query) {
    sql += ` AND (p.title LIKE ? OR b.name LIKE ?)`;
  ...
  sql += ` ORDER BY p.is_flash DESC, p.price_usd ASC`;

  const prodRes = await pool.query(sql, params, req);
  const products = prodRes.rows || [];
  
  let results = products.map(p => {
    const tasa = parseFloat(p.bcv_rate || 36.50);
    return {
      id: p.id,
      title: p.title,
      category: p.category,
