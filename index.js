...
// Estado global para saber si el Administrador tiene activo el modo Sandbox (Pruebas)
let adminSandboxMode = false;
let adminLoggedIn = false;

// Adaptador universal para Cloudflare D1 con soporte para Doble Base de Datos (Producción y Sandbox)
const d1Client = {
  async execute(sql, params = [], isSandbox = false) {
    ...
    if (!accountId || !databaseId || !apiToken) {
      throw new Error("Faltan las variables de entorno de Cloudflare D1 (CF_ACCOUNT_ID, CF_DATABASE_ID, CF_API_TOKEN)");
    }
...
const pool = {
  async query(text, params = [], req = {}) {
    let sql = text;
    let args = params;
    // El entorno está determinado exclusivamente por el interruptor interno del Administrador
    const isSandbox = adminSandboxMode;
...
async function conectarDB() {
  try {
    await inicializarTablasEn(false); // Producción
    await inicializarTablasEn(true);  // Sandbox
    console.log("🚀 Conectado exitosamente a Cloudflare D1 (Producción y Sandbox configurados)");
  } catch (e) {
...
// Layout unificado con Etiqueta Superior de Test (Solo visible si el admin activa el sandbox) y Botón de Admin
const tailwindLayout = (title, content, req = {}) => {
  return `
    ...
    <body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col justify-between relative">
      ...
      <!-- ETIQUETA SUPERIOR DE AVISO TEST (SOLO VISIBLE SI EL ADMIN TIENE EL SANDBOX ENCENDIDO) -->
      ${adminSandboxMode ? `
        <div class="bg-amber-500 text-slate-900 text-center font-black text-xs py-2 px-4 shadow-md uppercase tracking-wider sticky top-0 z-50 flex justify-center items-center gap-3">
          <span>⚠️ ZONA DE PRUEBAS (SANDBOX) ACTIVA • Base de Datos Aislada</span>
          <a href="/admin" class="bg-slate-900 text-white px-2.5 py-1 rounded-lg text-[10px] hover:bg-slate-800 transition shadow-sm">Ir a Controlar</a>
        </div>
      ` : ''}
      ...
      <a href="/admin" class="fixed bottom-6 right-6 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs px-4 py-3 rounded-full shadow-2xl flex items-center gap-2 z-50 border-2 border-rose-500 transition-all hover:scale-105">
        🛡️ Panel Admin ${adminSandboxMode ? '(Test Activo)' : ''}
      </a>
    ...
    <script>
      ...
      async function calcularRutaPresupuesto() {
        ...
        sugerencias.forEach(s => {
          html += \`<div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm"><span class="text-[10px] font-bold bg-rose-50 text-rose-600 px-2 py-0.5 rounded-full">Buscaste: \${s.buscado}</span><h5 class="font-bold text-slate-800 text-sm mt-1">\${s.title || s.encontrado}</h5>\${s.price_usd ? \`<p class="text-xs text-emerald-600 font-extrabold mt-1">$\${s.price_usd} (\${s.business_name})</p>\` : ''}</div>\`;
        });
      ...
      function obtenerGPSCliente() {
        if(navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(pos => {
            document.getElementById('gps_status').innerText = "✅ GPS Activo";
            buscar();
          });
        }
      }
      ...
      async function buscar() {
        ...
        if(json.data.length === 0) {
          ...
          json.data.forEach((item, index) => {
            html += \`
              <div class="bg-white rounded-2xl p-4 shadow-sm border \${index === 0 ? 'border-emerald-400 ring-2 ring-emerald-100' : 'border-slate-100'} relative">
                \${index === 0 ? '<span class="absolute top-3 right-3 bg-emerald-500 text-white text-[10px] font-extrabold px-2.5 py-1 rounded-full uppercase">🔥 Mejor Precio</span>' : ''}
...
app.get('/register', async (req, res) => {
  ...
  const body = `
    <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
      ...
      <h2 class="text-2xl font-extrabold text-slate-800 text-center mb-6">📝 Registro CercaYa</h2>
      <form action="/api/register" method="POST" class="space-y-4">
        ...
        <button type="submit" class="w-full bg-rose-600 text-white font-bold py-3 rounded-xl shadow-md">Registrarse</button>
      </form>
    </div>
    <script>
...
app.post('/api/register', async (req, res) => {
  const { email, password, full_name, plan_id, business_name, category, reference, phone, sec_question, sec_answer } = req.body;

  await pool.query(
    `INSERT INTO users (account_id, email, password, full_name, phone, role, status, expires_at, sec_question, sec_answer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ['CY-' + Math.floor(1000 + Math.random() * 9000), email, password, full_name, phone, parseInt(plan_id) === 1 ? 'user' : 'merchant', 'trialing', new Date(Date.now() + 14*86400000).toISOString(), sec_question, sec_answer],
    req
  );
  ...
  if (parseInt(plan_id) !== 1 && business_name) {
    await pool.query(
      ...
      [user.account_id, user.id, business_name, full_name, category || 'Otros', reference || 'Céntrico', '10.2241,-67.5871', phone, parseInt(plan_id), 36.50, 'trialing', new Date(Date.now() + 14*86400000).toISOString()],
      req
    );
  }
  res.redirect('/login');
...
app.get('/login', (req, res) => {
  res.send(tailwindLayout('Login', `
    <div class="max-w-md mx-auto bg-white rounded-3xl p-6 shadow-sm border mt-10">
...
app.get('/dashboard', async (req, res) => {
  ...
  const business = (bizRes.rows || [])[0];
  if (!business) return res.send("<h3>Acceso denegado.</h3>");
  ...
  let prodList = products.map(p => `
    <div class="py-3 border-b text-sm space-y-2">
      <div class="flex justify-between">
        <span class="font-bold text-slate-800">${p.title}</span>
        <span class="font-extrabold text-emerald-600">$${p.price_usd}</span>
      </div>
      <form action="/api/merchant/product/edit" method="POST" class="bg-slate-50 p-2 rounded-xl flex gap-2">
        <input type="hidden" name="product_id" value="${p.id}"><input type="hidden" name="user_id" value="${userId}">
        <input type="text" name="title" value="${p.title}" class="flex-1 bg-white border rounded px-2 py-1 text-xs" required>
        <input type="number" step="0.01" name="price_usd" value="${p.price_usd}" class="w-20 bg-white border rounded px-2 py-1 text-xs" required>
        <button type="submit" class="bg-slate-800 text-white text-[10px] font-bold px-3 py-1 rounded">💾</button>
      </form>
    </div>
  `).join('');
  ...
  res.send(tailwindLayout('Panel Negocio', `
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-3xl p-6 shadow-sm border">
        <h3 class="text-base font-bold mb-2">Inventario</h3>
        <div>${prodList || '<p class="text-xs text-slate-400">Sin productos.</p>'}</div>
      </div>
    </div>
  `, req));
});

// ==================== PANEL ADMIN PROTEGIDO Y GESTIÓN DE ENTORNO ====================
app.get('/admin', async (req, res) => {
  // Si no ha iniciado sesión en admin, se muestra el formulario de ingreso seguro con nombre de usuario y contraseña
  if (!adminLoggedIn) {
    return res.send(tailwindLayout('Login Admin', `
      <div class="max-w-md mx-auto bg-white rounded-3xl p-6 shadow-sm border mt-10 space-y-4">
        ...
        <form action="/api/admin/login" method="POST" class="space-y-3">
          ...
          <button type="submit" class="w-full bg-slate-900 text-white font-bold py-3 rounded-xl">Entrar al Panel</button>
        </form>
      </div>
    `, req));
  }

  const citRes = await pool.query('SELECT * FROM users WHERE role = ?', ['user'], req);

  let listHtml = (citRes.rows || []).map(u => `
    <div class="bg-slate-50 border rounded-2xl p-4 mb-3">
      <h4 class="font-bold text-slate-800">${u.full_name} (${u.account_id})</h4>
  ...
  res.send(tailwindLayout('Admin Panel', `
    <div class="space-y-6">
      <div class="bg-white rounded-3xl p-6 shadow-sm border flex justify-between items-center">
        <div>
          <h2 class="text-2xl font-extrabold text-slate-800">🛡️ Admin CercaYa</h2>
          <p class="text-xs text-slate-500">Entorno actual: <b class="${adminSandboxMode ? 'text-amber-600' : 'text-emerald-600'}">${adminSandboxMode ? 'SANDBOX (Pruebas)' : 'PRODUCCIÓN REAL'}</b></p>
        </div>
        <!-- SWITCH / INTERRUPTOR DE ENTORNO -->
        <form action="/api/admin/toggle-env" method="POST">
          <button type="submit" class="font-bold text-xs px-4 py-3 rounded-2xl shadow transition ${adminSandboxMode ? 'bg-amber-500 hover:bg-amber-600 text-white' : 'bg-emerald-600 hover:bg-emerald-700 text-white'}">
            ${adminSandboxMode ? '🟢 Apagar Sandbox (Ir a Prod)' : '⚪ Encender Sandbox (Test)'}
          </button>
        </form>
      </div>

      ${adminSandboxMode ? `
        <div class="bg-amber-50 border border-amber-200 rounded-3xl p-5 space-y-3">
          <h3 class="font-extrabold text-amber-800 text-sm">🧪 Acciones y Botón de Reinicio de Sandbox</h3>
          <div class="flex gap-2 flex-wrap">
            <form action="/api/admin/seed-test" method="POST">
              <button type="submit" class="bg-amber-600 text-white font-bold text-xs px-4 py-2.5 rounded-xl">🚀 Cargar Datos Test</button>
            </form>
            <form action="/api/admin/reset-test" method="POST" onsubmit="return confirm('¿Seguro que deseas limpiar por completo la base de datos de prueba?')">
              <button type="submit" class="bg-slate-900 text-white font-bold text-xs px-4 py-2.5 rounded-xl">🧹 Botón de Reinicio de Sandbox</button>
            </form>
          </div>
        </div>
      ` : ''}
      ...
      <div class="bg-white rounded-3xl p-6 shadow-sm border">
        <h3 class="text-base font-bold mb-4">Cuentas Ciudadanas en este Entorno</h3>
        <div>${listHtml || '<p class="text-xs text-slate-400">No hay cuentas.</p>'}</div>
      </div>
...
// Validación de credenciales de administrador (Modifica "admin" y "tucuenta123" por tus datos deseados)
app.post('/api/admin/login', (req, res) => {
  const { username, password } = req.body;
  if (username === 'admin' && password === 'tucuenta123') {
    adminLoggedIn = true;
...
// Interruptor para alternar el sandbox
app.post('/api/admin/toggle-env', (req, res) => {
  if (adminLoggedIn) {
    adminSandboxMode = !adminSandboxMode;
  }
  res.redirect('/admin');
});
...
// Endpoints de Sandbox D1
app.post('/api/admin/seed-test', async (req, res) => {
  if (adminLoggedIn && adminSandboxMode) {
    await pool.query(`INSERT INTO users (account_id, email, password, full_name, phone, role, status, expires_at, sec_question, sec_answer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, ['CY-TEST', 'test@cercaya.com', '123456', 'Tester', '04120000000', 'user', 'active', new Date().toISOString(), 'Color', 'Azul'], req);
  }
  res.redirect('/admin');
});
...
app.post('/api/admin/reset-test', async (req, res) => {
  if (adminLoggedIn && adminSandboxMode) {
    ...
    await pool.query("DELETE FROM users WHERE email != 'admin@admin.com'", [], req);
  }
  res.redirect('/admin');
});
...
app.post('/api/merchant/product', async (req, res) => {
  ...
  await pool.query('INSERT INTO products (business_id, title, category, price_usd, status, is_flash) VALUES (?, ?, ?, ?, ?, ?)', [business_id, title, category, parseFloat(price_usd), 'disponible', 0], req);
...
app.post('/api/merchant/product/edit', async (req, res) => {
  ...
  await pool.query('UPDATE products SET title = ?, price_usd = ? WHERE id = ?', [title, parseFloat(price_usd), product_id], req);
  res.redirect(`/dashboard?user_id=${user_id}`);
});
...
app.get('/api/search', async (req, res) => {
  ...
  const bizRes = await pool.query("SELECT * FROM businesses WHERE status != 'blocked'", [], req);
...
app.listen(PORT, '0.0.0.0', () => { console.log(`🚀 CercaYa (Cloudflare D1 Multi-Entorno Seguro) activo en puerto ${PORT}`); });
